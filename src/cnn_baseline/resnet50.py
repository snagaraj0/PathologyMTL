...
LR = 0.0035
EPOCHS = 10
opt = optim.Adam(resnet.parameters(), LR)
...
def train_loop(train_loader, valid_loader, opt):
    ...
    for epochs in tqdm(range(EPOCHS)):
        ...
        if best_acc < val_acc:
            torch.save(resnet.state_dict(), os.path.join(f"/home/ubuntu/cs231nFinalProject/src/cnn_baseline/resnet_params_{epochs+1}.pth"))
            best_acc = val_acc  

    # Save params after training
    torch.save(resnet.state_dict(), os.path.join("/home/ubuntu/cs231nFinalProject/src/cnn_baseline/resnet_final_params.pth"))
    return train_loss
