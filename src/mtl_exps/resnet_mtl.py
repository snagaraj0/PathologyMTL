        ...
        for h in range(rows):
            for w in range(rows):
                ...
                if self.transform is not None:
                   ...
                   curr_mask = self.transform(image=curr_mask)['image']
...
#    # Only let last block of encoder train
...
def train_loop(train_loader, valid_loader, opt):
    ...
    for epochs in tqdm(range(EPOCHS)):
        ...
        if best_acc < val_class_acc and best_dice < dice_score:
            torch.save(mtl.state_dict(), os.path.join(f"/home/ubuntu/cs231nFinalProject/src/mtl_exps/resnet_mtl_{epochs+1}.pth"))
            best_acc = val_class_acc
            best_dice = dice_score

    # Save params after training
    torch.save(mtl.state_dict(), os.path.join("/home/ubuntu/cs231nFinalProject/src/mtl_exps/resnet_mtl_final_params.pth"))
    return train_loss
