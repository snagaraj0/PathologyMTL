...
from densenetMTL import MTLWithDensenet121Encoder
...
for name, param in mtl.named_parameters():
    ...
    if name.startswith('down_blocks') or name.startswith('input_block'):
        param.requires_grad = False
    # Freeze all down except last block
    if name.startswith('down_blocks.7'):
        param.requires_grad = True
"""
...
def train_loop(train_loader, valid_loader, opt):
    ...
    for epochs in tqdm(range(EPOCHS)):
        ...
        if best_acc < val_class_acc and best_dice < dice_score:
            torch.save(mtl.state_dict(), os.path.join(f"/home/ubuntu/cs231nFinalProject/src/mtl_exps/densenet_mtl_{epochs+1}.pth"))
            best_acc = val_class_acc
            best_dice = dice_score

    # Save params after training
    torch.save(mtl.state_dict(), os.path.join("/home/ubuntu/cs231nFinalProject/src/mtl_exps/densenet_mtl_final_params.pth"))
    return train_loss
...
def test_loop(loader):
    ...
    with torch.no_grad():
        for (data, masks, label) in tqdm(loader):
            ...
            # MTL loss function
            loss = class_loss_func(test_class_logits, label) + segment_loss_func(test_mask_logits, masks) + 1e-6 
